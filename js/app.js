...
function updateAdminUI() {
  ...
  if (adminNav) {
    adminNav.textContent = isAdmin ? '⚙ 관리자' : '🔑 관리자';
  }
}
...
async function renderMembersPage() {
  ...
  try {
    ...
    listWrap.innerHTML = `<div class="empty-state"><p>⚠ 데이터를 불러오지 못했습니다: ${e.message}</p></div>`;
  }
}
...
function renderMemberTable(members) {
  ...
  if (!members.length) {
    wrap.innerHTML = `<div class="empty-state"><div class="empty-icon">👥</div><p>등록된 회원이 없습니다.</p></div>`;
    return;
  }
  wrap.innerHTML = `
...
window.submitEditMember = async function(id) {
  ...
  try {
    await DB.updateMember(id, phone, {
      ...
      dept: val('edit-dept'),
      studentId: val('edit-studentid'),
      company: val('edit-company'),
      position: val('edit-position')
    });
...
window.submitAddMember = async function() {
  ...
  try {
    await DB.addMember({
      ...
      dept: val('inp-dept'),
      studentId: val('inp-studentid'),
      company: val('inp-company'),
      position: val('inp-position')
    });
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const meetings = meetingMap[d] || [];
    const cls = `cal-day${isToday ? ' today' : ''}${dow === 0 ? ' sunday' : dow === 6 ? ' saturday' : ''}`;

    html += `<div class="${cls}" onclick="onCalDayClick(${d})">
      <div class="day-num">${d}</div>
      ${meetings.map(m => `<span class="cal-event-dot" onclick="event.stopPropagation(); openMeetingModal('${m.id}')">${m.place}</span>`).join('')}
    </div>`;
  }
...
function renderMeetingList() {
  ...
  if (upcoming.length) {
    ...
    html += upcoming.map(m => meetingCardHTML(m)).join('');
  }
  if (past.length) {
    ...
    html += past.map(m => meetingCardHTML(m)).join('');
  }
  wrap.innerHTML = html;
}
...
window.openMeetingModal = async function(id) {
  ...
  const teamResult = await DB.getTeamResult(id);
      ...
      ${participants.length ? participants.map(p => `
        <div class="participant-item">
          <div>
            ...
            <div class="p-info">${p.phone}</div>
          </div>
          <span class="badge badge-green">신청완료</span>
        </div>`).join('') : '<div class="empty-state" style="padding:20px"><p>아직 신청자가 없습니다.</p></div>'}
    </div>
    ...
    <div style="margin-top:24px; padding-top:20px; border-top:1px solid var(--line);">
      ...
      <div id="tab-join">
        <div class="form-grid" style="grid-template-columns:1fr 1fr; gap:12px;">
          <div class="form-group"><label>이름</label><input id="join-name" placeholder="홍길동"></div>
          <div class="form-group"><label>전화번호</label><input id="join-phone" placeholder="010-0000-0000"></div>
        </div>
      ...
      <div id="tab-cancel" style="display:none">
        <div class="form-grid" style="grid-template-columns:1fr 1fr; gap:12px;">
          <div class="form-group"><label>이름</label><input id="cancel-name" placeholder="홍길동"></div>
          <div class="form-group"><label>전화번호</label><input id="cancel-phone" placeholder="010-0000-0000"></div>
        </div>
...
window.doTeamMatch = async function(meetingId) {
  ...
  if (participants.length < 2) { showToast('참가자가 2명 이상이어야 매칭이 가능합니다.', 'error'); return; }
...
window.submitAddMeeting = async function() {
  ...
  try {
    await DB.addMeeting({
      date, place, time,
      note: document.getElementById('inp-meeting-note').value.trim()
    });
